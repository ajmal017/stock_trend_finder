...
Ticker.watching.each do |ticker|
  ...
  begin
  download_file = open(File.join(Dir.getwd, 'downloads', "#{ticker.symbol}_prices_yahoofinance.csv"), 'w:ASCII-8BIT')
  download_file.write(open(
      "http://ichart.finance.yahoo.com/table.csv?s=#{ticker.symbol}&d=10&e=14&f=2013&g=d&a=8&b=2&c=2001&ignore=.csv"
      #"http://www.google.com/finance/historical?q=#{ticker.symbol}&startdate=1%2F1%2F80&enddate=Nov+8%2C+2013&output=csv"
      ).read)
  download_file.close
  rescue
    puts "Error occurred on #{ticker.symbol}"
  end
