module ReportHelper
  RJUST_FIELDS=[:last_trade, :pct_change, :pct_above_52, :gap_percent, :float, :float_percent, :volume, :volume_ratio, :average_volume, :short_ratio]
  ...
  def set_css_class(report, field)
    ...
    case field
      ...
      when :float
        ...
        report[:float] = '%.0f' % (BigDecimal(report[:float]) / 1000).truncate(2) if report[:float].present?
